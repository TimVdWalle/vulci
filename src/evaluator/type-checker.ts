...
import { FunctionCall, FunctionDeclaration, TypeAnnotation, TypeMember } from "../ast.js";
...
export abstract class TypeChecker extends EvaluatorContext {
  protected assertParameterType(declaration: FunctionDeclaration, parameter: Token, parameterType: TypeAnnotation | null, value: RuntimeValue, callExpression: FunctionCall): void {
    if (parameterType === null || this.valueMatchesType(value, parameterType)) return;
    throw new Error(
  ...
  protected assertReturnType(declaration: FunctionDeclaration, value: RuntimeValue): void {
    if (declaration.returnType === undefined || this.valueMatchesType(value, declaration.returnType)) return;
    const location = declaration.returnType.members[0]?.token ?? declaration.name;
    throw new Error(
  ...
  protected valueMatchesType(value: RuntimeValue, annotation: TypeAnnotation): boolean {
    return annotation.members.some((member) => this.valueMatchesMember(value, member));
  }
  ...
  private valueMatchesMember(value: RuntimeValue, member: TypeMember): boolean {
    if (member.type === "TupleType") {
      return value.type === "Tuple" &&
        ...
        value.members.every((item, index) => this.valueMatchesType(item, member.members[index]!));
    }

    switch (member.lexeme) {
      case "any": return true;
      case "int": return value.type === "Integer";
      case "bool": return value.type === "Boolean";
      case "null": return value.type === "Null";
      case "str": return value.type === "String";
      case "list":
      case "set":
