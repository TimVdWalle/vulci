...
test("parses multiplication, division, and remainder left-associatively", () => {
    ...
    assert.equal(
        statement.expression.left.operator.type,
        TokenType.Star,
    );

    assert.equal(
        statement.expression.left.left.type,
        "BinaryExpression",
    );
    ...
    assert.equal(
        statement.expression.left.left.operator.type,
        TokenType.Slash,
    );
});
...
test("parses negated parenthesized expressions", () => {
    ...
    assert.equal(
        statement.expression.operand.type,
        "BinaryExpression",
    );
});
